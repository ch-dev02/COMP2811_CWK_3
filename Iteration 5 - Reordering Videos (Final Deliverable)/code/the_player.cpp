...
void ThePlayer::shuffleVideos(){
    ...
    std::shuffle(infos->begin(), infos->end(), rng);
    renderList();
    for(int i = 0; i < infos->size(); i++){
        if(QString::compare(infos->at(i).url, QString(""), Qt::CaseInsensitive) != 0){
            switchTo(i);
            break;
        }
    }
...
void ThePlayer::moveVideoUp(int index){
    if(index != 0){ // if not first
        std::vector<TheButtonInfo>* ninfos = new std::vector<TheButtonInfo>();

        // find index of previous video which is still active (not removed)
        int prev = -1;
        ...
        //duplicate vector but swap index and prev infos
        for (int i=0; i < count; i++) {
            if(i == index && prev != -1){
                ninfos->push_back(TheButtonInfo(infos->at(prev).url));
            }else if (i == prev){
                ninfos->push_back(TheButtonInfo(infos->at(index).url));
            }else{
                ninfos->push_back(TheButtonInfo(infos->at(i).url));
            }
        }
...
void ThePlayer::moveVideoDown(int index){
    if(index != (count - 1)){ // if not last
        std::vector<TheButtonInfo>* ninfos = new std::vector<TheButtonInfo>();
        ...
        for (int i=0; i < count; i++) {
            if(i == index && next != -1){
                ninfos->push_back(TheButtonInfo(infos->at(next).url));
            }else if (i == next){
                ninfos->push_back(TheButtonInfo(infos->at(index).url));
            }else{
                ninfos->push_back(TheButtonInfo(infos->at(i).url));
